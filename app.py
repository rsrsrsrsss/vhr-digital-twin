...
# ==========================================================
    ...
    # База данни с физико-химични и механични характеристики на сплавите
    metal_info_db = {
        ...
        "Аустенитна стомана 12Х18Н10Т (Спомагателни системи & Колектори)": {
            ...
            "sigma_dov": 137.0,
            "desc": "Материал за спомагателни тръбопроводи на първи контур, колектори на парогенератори и системи за пречистване на топлоносителя (СВО-1)."
        },
    ...
    with st.expander(f"ℹ️ Подробна техническа характеристика за: {metal}", expanded=True):
        col_m1, col_m2, col_m3 = st.columns([1.5, 2, 1.5])
        col_m1.markdown(f"**Тип сплав:**  \n`{selected_info['type']}`")
        col_m2.markdown(f"**Химически състав:**  \n`{selected_info['comp']}`")
        col_m3.markdown(f"**Допустимо напрежение $[\sigma]$:**  \n`{selected_info['sigma_dov']} MPa` (при 300°C)")
