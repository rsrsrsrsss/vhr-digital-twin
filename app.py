...
# ==========================================================
# МОДУЛ 2: ЛАБОРАТОРИЯ & ОРАЗМЕРЯВАНЕ НА СЪОРЪЖЕНИЯ
# ==========================================================
    ...
    # --- 1. Избор на тип съоръжение и материал ---
    ...
    # Допустими напрежения за стоманите [MPa]
    allowable_stress_db = {
        "Конструкционна стомана 10ГН2МФА (Главни циркулационни тръбопроводи - ГЦТ)": 185.0,
        "Корпусна стомана 15Х2МФА / 15Х2НМФА (Корпус на Реактора & Компенсатор)": 195.0,
        "Аустенитна стомана 08Х18Н10Т (Първи контур / Тръби Парогенератор)": 137.0,
        "Аустенитна стомана 12Х18Н10Т (Спомагателни системи & Колектори)": 137.0,
        "Антикорозионен наплав 04Х20Н10Г2Б (Вътрешна защита на ГЦТ)": 145.0,
        "Въглеродна стомана 20К / 16ГС (Втори контур / Паропроводи)": 115.0,
        "Високоякостна стомана 25Х1МФ / 38ХН3МФА (Главни шпилки)": 210.0,
        "Циркониева сплав Э110 / Э635 (Обвивки на ТВЕЛ)": 95.0
    }

    # --- 2. Геометрия според избора ---
    st.markdown("---")
    ...
    if "Тръбопровод" in equipment_type:
        ...
        # Автоматично изчисляване на диаметъра D за дадения обем
    ...
    if "Цилиндричен съд" in equipment_type:
        st.success(
            ...
            * **Репоръка за изпитване:** Хидравлично изпитване на якост при налягане $P_{{test}} = 1.25 \\cdot {pressure_bar} = {1.25 * pressure_bar:.1f} \\text{{ bar}}$.
            """
        )
    else:
        # Загуба на метален тонаж за тръбопровода
        ...
        st.success(
            ...
            * **Дължина на трасето ($L$):** **{pipe_length_m} m** | **Вътрешен диаметър ($D$):** **{diameter_mm} mm**
